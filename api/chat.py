...
JOB_SITUATIONS = {
    ...
    "vip": {
        1: ["VIP 라운지 이용 횟수 및 동반 입장 인원 문의", "발레파킹 이용 방법 문의", "VIP 전용 행사 초청 여부 문의", "퍼스널 쇼퍼 서비스 예약 방법 문의", "라운지 내 다과/음료 메뉴 문의"],
        2: ["발레파킹 신청했는데 대기가 너무 길다고 항의", "라운지 이용 횟수가 다 찼다고 항의", "VIP 등급인데 라운지 입장이 거절됐다고 항의", "명절 선물이 작년보다 품질이 떨어졌다고 항의", "포인트(세일리지)가 예상보다 적게 적립됐다고 항의"],
        3: ["일반 고객과 같은 라운지를 쓴다며 강하게 항의", "라운지에서 직원 응대가 불친절했다며 항의", "VIP 전용 행사에 초청받지 못했다며 항의", "등급에 비해 혜택이 너무 적다며 강하게 항의", "라운지 대기 시간이 너무 길다며 강하게 항의"],
        4: ["VIP 등급 강등됐다며 강력 항의 및 보상 요구", "타 백화점 VIP 서비스와 비교하며 혜택 개선 압박", "발레파킹 차량에 스크래치 났다며 보상 요구", "퍼스널 쇼퍼 서비스 수준이 너무 낮다며 책임자 요구"],
        5: ["VIP 탈퇴 선언하며 주변 VIP 회원 이탈 협박", "SNS에 VIP 서비스 실태 폭로하겠다고 협박", "본사 임원에게 직접 민원 넣겠다고 협박", "소비자원에 백화점 VIP 등급 조작 신고하겠다고 협박"],
    },
    "rental": {
...
CUSTOMER_TYPES = [
    ...
    {"type": "합리적 소비자(30~40대)", "desc": "가격 대비 가치를 꼼꼼히 따지는 30~40대. 논리적으로 근거를 들며 주장함. 규정을 잘 알고 있어서 허점을 파고듦"},
...
- *(행동묘사)* 같은 별표 액션 표현 절대 금지. 오직 실제 대화 텍스트만 출력할 것
...
# 직무별 어울리는 고객유형 인덱스 (CUSTOMER_TYPES 순서 기준)
# 0:급한직장인 1:꼼꼼한주부 2:어르신 3:외국인 4:화난단골 5:젊은첫방문 6:VIP고객 7:합리적소비자 8:감정적고객 9:억지고객
JOB_CUSTOMER_MAP = {
    "cashier":   [0,1,2,3,4,5,7,8,9],      # 모든 유형 가능
    "vegetable": [1,2,3,4,5,7,8,9],         # 주로 주부/어르신/단골
    "seafood":   [1,2,4,7,8,9],              # 주로 주부/어르신/단골
    "meat":      [1,2,4,7,8,9],              # 주로 주부/어르신/단골
    "cs":        [0,1,2,4,5,7,8,9],          # 다양한 유형
    "vip":       [1,4,6,7,8,9],              # VIP 고객/단골/꼼꼼한 주부 위주
    "rental":    [1,2,5,8,9],                # 젊은 부모/어르신/감정적 고객
    "general":   [0,1,2,3,4,5,7,8,9],       # 모든 유형 가능
}
